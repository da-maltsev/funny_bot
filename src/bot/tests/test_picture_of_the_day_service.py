...
async def test_all_calls_were_made(service, mocker, mock_open_ai_call, mock_photo_name, mock_remove_file, mock_send_photo):
...
async def test_all_calls_were_made_no_holiday(service, mocker, mock_open_ai_call, mock_photo_name, mock_remove_file, mock_send_photo):
    mock_get_holiday = mocker.patch(
        "clients.holiday.client.HolidayClient.get_list_of_holidays",
        return_value=[
            (
